...
(function() {
  ...
  const isInteractive = urlParams.get('interactive') == 1 ? true : false
  ...
  function walkBackward(stepId, update_url) {
    ...
    const idxNext = getCurrentStoryStepIds().indexOf(stepId) + 1
    currentStory[idxNext].scrollIntoView({behavior: 'smooth'})
    while (currentStory.length > idxNext) {
      let lastStep = currentStory.pop()
      lastStep.classList.add('fade-out-bck')
      setTimeout(() => lastStep.remove(), 1100)
    }
    update_url ? updateURL() : null
    return currentStory[currentStory.length - 1]
  }
  ...
  if (initSteps) {
    initSteps = initSteps.split(',').map(addStep)
    setTimeout(
  ...
  window.addEventListener('popstate', function(ev) {
    console.log({event: 'popstate', state: ev.state})
