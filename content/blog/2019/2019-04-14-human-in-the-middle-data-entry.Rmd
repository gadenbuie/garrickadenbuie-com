...
```{r, eval=FALSE}
speak <- function(x, speak_command = "say", args = NULL) {
  if (inherits(x, "data.frame")) {
    # read a data.frame row-wise like "column name: value. ..."
    for (name in names(x)) {
  ...
  for (thing in x) {
    if (!is.null(args)) {
      # additional arguments passed to the speak command
      args <- paste0(" ", paste(args, collapse = " "))
    }
    system2(speak_command, paste0("'", thing, "'", args))
