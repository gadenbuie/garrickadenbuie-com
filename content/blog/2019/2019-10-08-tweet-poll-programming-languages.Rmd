...
tags:
  ...
  - Poll
description: >
  A deep dive into an informal, free-form survey about 
...
(because they're friends and language wars as silly),
...
`r tweets %>% filter(!is.na(reply_to_status_id) | is_quote) %>% nrow() %>% format(big.mark = ',')`
people replied to or quoted a tweet,
thus contributing the popularity of the poll.
Here are the top 10 recipients of a reply or quote tweet.
...
Which languages were learned first
compared with the languages that one would recommend 
to others to learn first? 
Here many people started with older languages
...
as many people indicated that it caused them difficulties.
...
tweets_r_counted <-
  ...
  ungroup() %>%
  filter(answer == "r") %>%
  mutate(Rank = if_else(total < 10, "", paste0(Rank))) %>%
  mutate(group = if_else(n == 1 & Rank == "", "single", category2)) %>% 
  select(-total) %>% 
  nest(data = category2) %>% 
  mutate(
  ...
  select(Category = category, Rank, Total = n)
```

How do developers feel about my favorite language?
R isn't a typical first language,
but it is among the top 20 recommended to be a first language.
It's also the
