...
```{r load-pt-mono, echo=FALSE}
htmltools::tags$link(href = "ttps://fonts.googleapis.com/css?family=PT+Mono",
                     ref = "stylesheet")
```
...
exec_time <-
  ...
  mutate(
    ...
    top_category = recode(top_category, !!!executive_categories),
    top_category = factor(top_category, levels = executive_categories)
  ) %>% 
  mutate(
    # Create text labels from original data
    label_title    = glue::glue("<b>{top_category}</b>"),
    label_subtitle = if_else(!is.na(listed_title) & 
                               tolower(top_category) != tolower(listed_title), 
                             glue::glue("<br><em>{listed_title}</em>"), ""),
    label_location = if_else(!is.na(listed_location), 
                             glue::glue("<br><em>{listed_location}</em>"), ""),
    label_time     = glue::glue("<br><br>{strftime(time_start, '%A, %B %e %H:%M')} ", 
                                "to {strftime(time_end, '%H:%M')}"),
    label_notes    = if_else(!is.na(notes), paste0("<br><br>", notes), ""),
    label = paste0(label_title, label_subtitle, label_location, label_time, label_notes)
  ) %>% 
  mutate(
...
djt_time <-
  ...
  distinct(status_id, .keep_all = TRUE) %>% 
  filter(
    created_at >= min(exec_time$time_start),
    created_at <= max(exec_time$time_end)
  ) %>% 
  mutate(label = glue::glue("{text}<br>{strftime(created_at, '%F %T')}"), 
         date = floor_date(created_at, "day")) %>%
  select(-text) %>% 
  filter_workday(start_var = created_at, end_var = created_at) %>% 
  mutate_at(vars(created_at), in_hours)
...
djt_joined_all <- 
  ...
  select(event_id, starts_with("time_"), created_at, listed_title, top_category, text) %>% 
  filter(
    !is.na(text)
  ) %>% 
  ...
  mutate(
    ...
    label_sched = if_else(
      ...
      glue::glue("<br>During: <b>Weekend</b>"),
      glue::glue(
        "<br>During: <b>{top_category}</b>",
        "<br>Sched: {strftime(time_start, '%I:%M')} ",
        "to {strftime(time_end, '%I:%M %p')}")
    ),
    label = glue::glue("{str_wrap(text, 50)}","
                       <br><i>@realDonaldTrump {strftime(created_at, '%a, %b %e, %I:%M %p')}</i>",
                       "{label_sched}"
                       )
  ) %>%
...
```{r plot-daily-schedule, fig.show = "show"}
g <-
  exec_time %>%
...
g_subdued <- 
  ...
  geom_point(
    data = djt_workday,
    aes(y = hour, x = date + 3600 * 12, text = label),
    color = "#2c3741",
    size = 0.8
  )
...
# remove hover labels for time category shapes
...
```{r eval=FALSE}
#lightbox_img(knitr::fig_chunk("plot-daily-schedule", "png"))`
```
