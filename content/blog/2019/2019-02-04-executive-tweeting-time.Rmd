...
filter_workday <- function(df,  start = hms::hms(0, 0, 8), end = hms::hms(0, 0, 17)) {
  df %>% 
    filter(
      date(time_start) == date(time_end),
      hms::as.hms(time_start) >= start, 
      hms::as.hms(time_end) <= end
    ) 
}
...
```{r plot-emotion, fig.height=6, fig.show=TRUE}
# djt_sentiment %>%
...
djt_sentiment %>%
  ...
  # ggridges::geom_density_ridges() +
  geom_boxplot(alpha = 0.8, color = "grey20", outlier.shape = NA) +
  scale_fill_manual(values = event_type_colors) +
