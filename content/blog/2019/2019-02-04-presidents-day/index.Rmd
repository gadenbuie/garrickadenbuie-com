...
We can get a sense of the timing of Trump's tweeting activities by looking at the time of day of each tweet and the schdeled activity that's going on at the time.
...
```{r tweets-by-exec-time, fig.height=5, echo=FALSE}
djt_joined %>%
  group_by(date) %>%
  count() %>%
  rename(tweets = n) %>%
