...
```{r plot-time-category-total-pct, fig.height=4}
...
`r lightbox_img(knitr::fig_chunk("plot-time-category-pct", "png"))`

In fact, the largest non-executive time block for the
`r exec_time_n_weekdays - exec_time_gt_half`
days where executive time isn't more than half of the workday are almost entirely travel related.

```{r table-not-full-on-exec-time-day, results="asis"}
exec_time_summary %>%
...
My guess is that this is in part due to travel being the only activity with a long-enough duration to displace executive time, and also (speculatively) in part that travel probably most resembles executive time.

```{r plot-exec-time-vs-others, fig.width=8, fig.height=3.5}
exec_time_total %>%
  filter(top_category != "Unknown") %>%
...
## How do the President's Tweets Relate to His Schedule?
...
ggplot(djt_joined_work_non_work) + 
  ...
  theme(
    ...
    axis.text.y = element_blank(),
    strip.text.y = element_text(angle = 180, margin = margin(r = 25), hjust = 1),
...
What's striking is that it's nearly impossible to tell the difference between early morning tweeting and the start of President Trump's official workday at 8am.

```{r plot-tweets-work-non-work-same-timeline, fig.height=7}
# g_work_non_work_tweet_timeline <- 
  djt_joined_work_non_work %>% 
  filter(top_category %in% c("Non-Work Hours", "Executive Time")) %>% 
  mutate(week = floor_date(created_at, "week"),
         week = strftime(week, "%F")) %>% 
  # filter(week %in% unique(week)[1:15]) %>% 
  ggplot() + 
  aes(x = hour, y = 1, color = top_category) +
...
```{r tweet_sentiment, eval = FALSE, fig.height=8}
