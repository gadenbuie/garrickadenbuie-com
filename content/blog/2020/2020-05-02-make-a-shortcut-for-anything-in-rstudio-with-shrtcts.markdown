...
[shrtcts]{.pkg} now has [a dedicated site](https://pkg.garrickadenbuie.com/shrtcts),
...
and use [roxygen2]{.pkg} documentation strings to [configure your shortcuts](https://pkg.garrickadenbuie.com/shrtcts/articles/shrtcts.html#shrtcts-r-format).
...
for your [shrtcts]{.pkg} shortcuts directly in the configuration R script.
...
run `add_rstudio_addin()` from [shrtcts]{.pkg}
...
with your [shrtcts]{.pkg} shortcuts.

First, if you re-install or update [shrtcts]{.pkg},
...
[shrtcts]{.pkg} comes with a demonstration `.shrtcts.yaml` file
...
(I was thinking of 45, by the way),
...
Whenever this happens, [shrtcts]{.pkg} has to re-wire
...
[rsam]{.pkg} provides 3 slots,
while [shrtcts]{.pkg} gives you 100.
