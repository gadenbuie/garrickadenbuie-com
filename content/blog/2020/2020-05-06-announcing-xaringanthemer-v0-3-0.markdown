...
that [xaringanthemer]{.pkg} can bring to your [xaringan]{.pkg} slides!
...
Equally exciting,
[xaringanthemer]{.pkg} finally has it's own hex logo
...
You can try out [xaringanthemer]{.pkg}
...
[xaringanthemer]{.pkg} works hand-in-hand with
[[xaringan]]{.pkg},
the R package that turns your R Markdown into beautiful HTML slides
rendered as web pages using the [remarkjs] library.

[xaringan]{.pkg} slides tend to look like the slides below,
and, by default,
[xaringan]{.pkg} uses the default slide theme from remarkjs.
...
[xaringan]{.pkg} does come with a collection of
[user-contributed themes][xaringan-themes]
to choose from,
but customizing any [xaringan]{.pkg} slide theme
...
The magic of [[xaringanthemer][xt]]{.pkg}
...
(the default fonts used by [xaringanthemer]{.pkg}
...
which starts from the default [xaringan]{.pkg} theme.
...
That's [xaringanthemer]{.pkg} in a nutshell!
...
The markdown syntax used by [xaringan]{.pkg}
...
[xaringanthemer]{.pkg} helps you add these additional colors to your theme.
...
Using the `red` color as an example, [xaringanthemer]{.pkg} adds
...
The named color and background classes are then easy to use in [xaringan]{.pkg}.
...
but your [[ggplot2]]{.pkg} plots stick out like a sore thumb
...
and [xaringanthemer]{.pkg} can help!
...
[xaringanthemer]{.pkg} provides a [ggplot2]{.pkg} theme
that knows the styles used in your slides:
`theme_xaringan()`.
Let's start with a basic, typical [ggplot2]{.pkg} plot of fuel efficiency of cars
...
It also uses the [[showtext]]{.pkg} by Yixuan Qiu
...
[xaringanthemer]{.pkg} also provide a ggplot2 theme for these slides as well:
`theme_xaringan_inverse()`.
...
[xaringanthemer]{.pkg} includes custom color and fill scales
based on the primary slide color.
This feature uses 
[`colorspace::sequential_hcl()`](http://colorspace.r-forge.r-project.org//reference/hcl_palettes.html) from the [[colorspace]]{.pkg} package.
...
or to use a [xaringanthemer]{.pkg} CSS file
...
This theme function can be used for complete [ggplot2]{.pkg} themes,
...
To learn more about the [ggplot2]{.pkg} themes,
...
If you like the **metropolis** theme in [xaringan]{.pkg} ---
...
that brings the title bar background to any [xaringan]{.pkg} presentation.
...
meaning that for any [xaringanthemer]{.pkg} slides
...
While the default [xaringan]{.pkg} fonts
