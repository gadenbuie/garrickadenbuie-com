...
**Update:** Yihui Xie (the author of `knitr` and `xaringan`) [pointed out on Twitter][yihui-tip] that another valid (and maybe better) option is to use `knitr::fig_chunk()` as [demonstrated here](#using-knitr-fig-chunk).
...
[Yihui Xie](https://yihui.name) pointed out [on Twitter][yihui-tip] that we can use the chunk option `fig.show="hide"` on the source chunk and then call `knitr::fig_chunk()` directly wherever we want to embed the plot.
