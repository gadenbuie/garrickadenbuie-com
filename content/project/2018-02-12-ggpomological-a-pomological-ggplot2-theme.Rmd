...
This base theme (more like an "out of the box" theme) sets the plot theme to be representative of the paper and styling of the watercolors and includes a paper-colored background. 
Two wrapper functions `theme_pomological_nobg()` and `theme_pomological_plain()` build on the first, just with a transparent (or white) background (for `_nobg`) and no crazy fonts or background (for `_plain`).
