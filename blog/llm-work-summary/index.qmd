...
```{markdown filename="prompt-system.md"}
{{< include prompt-system.md >}}
```
...
```{markdown filename="prompt-work-summary.md"}
{{< include prompt-work-summary.md >}}
```
...
work_data |>
  slice(15) |>
  epoxy_use_file("prompt-work-summary.md", .engine)
```

Here's an example of a single work summary using the 15th row.

`````html
`r epoxy::epoxy_use_file(work_data[15, ], "prompt-work-summary.md")`
`````
...
work_summary <-
  work_data |>
  epoxy_use_file("prompt-work-summary.md", .collapse = "\n\n")
```
...
#| cache: true
chat <- chat_claude(
  system_prompt = readLines("prompt-system.md")
)

work_summarized <-
  chat$chat(interpolate_file("prompt.md"), echo=FALSE)

# I'm caching the result so I don't rack up costs while I write this post
...
The summarization quality is no where near what I got from Claude, either,
